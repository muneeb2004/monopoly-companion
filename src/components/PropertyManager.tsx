...
import { X, ChevronDown, ChevronUp, Home, Ban } from 'lucide-react';
      ...
      <div className="bg-white w-full max-w-lg h-[90vh] sm:h-[80vh] rounded-t-2xl sm:rounded-2xl flex flex-col shadow-2xl animate-in slide-in-from-bottom-10 fade-in duration-300">
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {filteredProperties.map((property) => {
             ...
             // Skip non-purchasable spots for clarity unless showing "all" explicitly
             if (['special', 'chance', 'chest', 'corner', 'tax'].includes(property.type) && filter !== 'all') return null;

             return (
               <div key={property.id} className="border border-slate-100 rounded-lg bg-white shadow-sm overflow-hidden">
                 <div 
                    className="flex items-center gap-3 p-3 cursor-pointer hover:bg-slate-50 transition-colors"
                    onClick={() => property.price && setExpandedId(isExpanded ? null : property.id)}
                 >
                   <div className={cn("w-12 h-12 rounded-lg flex items-center justify-center font-bold text-xs text-center p-1 leading-none shadow-sm relative", GROUP_COLORS[property.group] || 'bg-slate-200')}>
                     {property.group === 'railroad' ? 'RR' : property.group === 'utility' ? 'UTIL' : ''}
                     {property.houses > 0 && (
                       <div className="absolute -top-1 -right-1 bg-green-500 text-white w-5 h-5 rounded-full text-[10px] flex items-center justify-center border-2 border-white">
                   ...
                   <div className="flex-1 min-w-0">
                     ...
                     <div className="text-xs text-slate-500">Price: ${property.price || 0}</div>
                   </div>
                   ...
                   {property.price ? (
                      <div className="flex items-center gap-2" onClick={e => e.stopPropagation()}>
                        <select
                          ...
                          className="text-xs border border-slate-200 rounded p-1 max-w-[100px] outline-none focus:border-blue-500"
                          style={{ 
                   ...
                   <div className="p-3 bg-slate-50 border-t border-slate-100 space-y-3">
                     ...
                     {property.houseCost && !property.isMortgaged && (
                        <div className="flex items-center gap-3 bg-white p-2 rounded-lg border border-slate-200">
                           <div className="flex items-center justify-center w-10 h-10 bg-green-100 text-green-700 rounded-lg">
                             <Home size={20} />
                           </div>
                           ...
                           <div className="flex flex-col gap-1">
                             ...
                             <button
                               ...
                               className="px-3 py-1 bg-red-100 text-red-700 text-xs font-bold rounded hover:bg-red-200 disabled:opacity-50"
                             >
                               Sell
                             </button>
                           </div>
