...
export const SettingsModal: React.FC<SettingsModalProps> = ({ isOpen, onClose }) => {
  ...
  const [sm, setSm] = useState<number>(startingMoney);
  const [pm, setPm] = useState<number>(priceMultiplier);
  const [rm, setRm] = useState<number>(rentMultiplier);
  ...
  useEffect(() => {
    setSm(startingMoney);
    setPm(priceMultiplier);
    setRm(rentMultiplier);
      ...
      <div className="bg-white rounded-2xl w-full max-w-md shadow-2xl flex flex-col overflow-hidden">
        ...
        <div className="p-4 space-y-4">
                ...
                {propOverrides.map(o => (
                  <div key={o.id} className="p-2 border-b last:border-b-0">
                    ...
                    <div className="flex gap-2 mt-2">
                      <input placeholder="Price override" value={o.priceOverride} onChange={(e) => setPropOverrides(prev => prev.map(p => p.id === o.id ? { ...p, priceOverride: e.target.value } : p))} className="p-1 border rounded w-32" />
                      <input placeholder="Rent override (comma-separated)" value={o.rentOverride} onChange={(e) => setPropOverrides(prev => prev.map(p => p.id === o.id ? { ...p, rentOverride: e.target.value } : p))} className="p-1 border rounded flex-1" />
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div>
            ...
            <input type="number" value={sm} onChange={(e) => setSm(Number(e.target.value))} className="w-full mt-2 p-2 border rounded" />
          ...
          <div>
            ...
            <input type="number" step="0.1" value={pm} onChange={(e) => setPm(Number(e.target.value))} className="w-full mt-2 p-2 border rounded" />
          ...
          <div>
            ...
            <input type="number" step="0.1" value={rm} onChange={(e) => setRm(Number(e.target.value))} className="w-full mt-2 p-2 border rounded" />
